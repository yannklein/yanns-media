...
export const MediaMap = ({
  ...
  accessToken: string;
  initialMedias: MediaWithImages[];
}) => {
  const mapRef = useRef<MapRef>(null);
  const [medias, setMedias] = useState<MediaWithImages[]>(initialMedias);
  const [selectedMedia, setSelectedMedia] = useState<MediaWithImages | null>(
    null,
  );
  ...
  const getMedias = async ({
    ...
    const res = await fetch(`/api/get-medias`, {
      method: 'POST',
      body: JSON.stringify({ person, year, event, id }),
    });
  ...
  const geojson: GeoJSON.FeatureCollection = {
    ...
    features: medias.map((media) => ({
      type: 'Feature',
      properties: {
        mediaId: media.images[0]?.id,
        mediaEvent: media.event,
  ...
  const onMapClick = (event: MapMouseEvent) => {
    ...
    if (clusterId !== undefined) {
      ...
      const clickedMedia = medias.find(
        (media) => media.images[0]?.id === feature?.properties?.mediaId,
      );
      if (clickedMedia) {
